  ...
  mbtips: {
    ...
    troubles: {
      "페이지 간 상태 종속성 제거로 UX 및 유지보수성 향상": {
        ...
        result:
          "사용자가 페이지를 원활하게 공유받을 수 있어 UX가 개선되었고, 개발자는 페이지 컴포넌트의 상태를 다른 컴포넌트에서 추적할 필요가 없어져 개발 편의성과 유지보수성이 함께 향상됨.",
      },
      "G4 이벤트 태깅 및 설문조사를 통한 UX 문제 진단": {
        problem:
...
무한 스크롤 구현과 HTML 콘텐츠를 마크다운 스타일로 렌더링하는 것이 주요 과제였습니다.`,
    ...
    troubles: {
      "에러 핸들링 강화로 명확한 사용자 피드백 제공": {
        problem:
          "서버에서 404, 500 오류 발생 시 사용자에게 아무런 안내가 없어 혼란 발생",
        progress:
          "HTTP 요청 시 서버에서 반환하는 상태 코드를 확인, Next.js의 커스텀 에러 페이지 기능(404.tsx, 500.tsx 등)을 활용해 해당 코드에 맞는 에러 페이지로 자동 리다이렉션",
        result:
          "사용자가 잘못된 URL에 접근하거나 서버에 문제가 발생했을 때 의미 없는 공백 화면 대신, 상황에 맞는 친절한 안내 페이지를 제공함으로써 혼란을 줄이고 신뢰감을 높여줌",
      ...
      "HttpOnly 쿠키 기반 인증 전환으로 XSS/CSRF 보안 강화": {
        problem:
          "AccessToken과 RefreshToken을 로컬 스토리지에 저장하고 있어, XSS로 인한 노출 위험 발생",
        progress:
          "로그인 시에는 토큰에 HttpOnly 옵션을 설정하여 보안을 강화하고, 인증이 필요한 요청은 클라이언트가 직접 백엔드 서버에 요청하는 대신, 서버의 API Route를 통해 프록시 방식으로 처리하도록 개선함.",
        result:
          "HttpOnly 쿠키는 JavaScript로 접근할 수 없기 때문에 XSS 공격으로부터 토큰이 탈취되는 일을 방지함. 인증이 필요한 모든 요청을 서버의 API Route에서 일괄적으로 처리하게 되므로 유지보수성 또한 증가",
      ...
      "사용자 입력 에러에 대한 UX 중심의 에러 핸들링 개선": {
        problem:
          "사용자 입력 오류 상황에서 정확한 피드백이 부족하여, 사용자 경험이 저해되고 페이지 이탈율이 증가하는 가능성이 존재하다는 사실을 발견",
        progress:
          "실시간 유효성 검사, 명확한 에러 메시지, 입력 형식 가이드 제공, 유효성 검사 등 사용자 입력 요소에서 제공해야 하는 규칙들을 재정의하고 리팩터링하여 에러에 대한 피드백을 보다 정밀히 함",
        result:
          "사용자 입력 오류에 대한 피드백이 명확해져, 사용자는 문제를 즉시 인지하고 빠르게 수정할 수 있게 됨. 사용 흐름이 끊기지 않으며, 결과적으로 페이지 이탈률을 줄이고 전반적인 UX가 향상됨",
...
상호작용 요소가 집중된 페이지 구조로 인해, 상태 분기 관리와 리렌더링 최소화가 핵심 과제였습니다.`,
    ...
    troubles: {
      "TanStack Query 기반 캐싱으로 비동기 요청 최적화": {
        ...
        progress:
          "Tanstacks Query의 useQuery 메서드를 사용하여 동일한 key로 useQuery가 호출되면, 이전에 받아온 데이터를 재사용하여 API 요청을 생략하도록 함",
        result:
          "평균 1.2초 소요되던 중복 API 요청들이 모두 제거되어 재방문 시 콘텐츠가 즉시 표시됨, 사용자 체감 속도가 개선됨",
      ...
      "Frontend-Fundamentals 가이드를 바탕으로 한 클린코드 작업": {
        problem:
          "특정 페이지에 주요 기능이 몰려 있어 페이지 컴포넌트가 관리하는 상태가 많아지고 JSX 또한 복잡해져 가독성이 현저히 저하된 상황",
        progress:
          "토스팀에서 제시한 Frontend-Fundamentals 클린코드 가이드를 바탕으로 리팩터링을 진행, 상태관리는 hook이 전담하고 JSX는 컴포넌트를 분리하여 가독성을 개선시킴",
        result:
          "상태는 Hook이 관리해서 필요한 값을 반환만 해주니, 페이지는 UI만 담당하게 됨 → 유지보수성과 가독성 향상, DX 개선",
